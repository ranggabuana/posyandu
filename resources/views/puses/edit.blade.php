<x-layout title="Edit PUS">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Data PUS</h2>
            <p class="text-sm text-gray-500">Perbarui data pasangan usia subur.</p>
        </div>
    ...
    <form action="{{ route('puses.update', $pus) }}" method="POST" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <select name="suami_id" id="select2-suami" class="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none">
                    ...
                    @foreach($pria as $p)
                        <option value="{{ $p->id }}" {{ old('suami_id', $pus->suami_id) == $p->id ? 'selected' : '' }}>{{ $p->nama }} (NIK: {{ $p->nik }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="istri_id" id="select2-istri" required class="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none">
                    ...
                    @foreach($wanita as $w)
                        <option value="{{ $w->id }}" {{ old('istri_id', $pus->istri_id) == $w->id ? 'selected' : '' }}>{{ $w->nama }} (NIK: {{ $w->nik }})</option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="keterangan" rows="3" class="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none" placeholder="Opsional...">{{ old('keterangan', $pus->keterangan) }}</textarea>
        ...
        <div class="flex justify-end">
            <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-500/30 transition-all flex items-center gap-2">
                <i class="mdi mdi-content-save"></i> Perbarui Data
            </button>
        </div>
    </form>
