<x-layout title="Data Posyandu">
    <x-page-header 
        title="Daftar Unit Posyandu"
        subtitle="Kelola data lokasi posyandu dan wilayah binaan RW di Desa Banjar"
    ...
    >
        <a href="{{ route('posyandus.export') }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-xs transition flex items-center gap-2 shadow-xs">
            <i class="mdi mdi-file-excel text-sm"></i> <span>Export Excel</span>
        </a>
        <a href="{{ route('posyandus.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs transition flex items-center gap-2 shadow-xs">
            <i class="mdi mdi-plus text-sm"></i> <span>Tambah Posyandu</span>
        </a>
    </x-page-header>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-4 border-b border-gray-200 bg-gray-50/50">
            <form id="filter-form" action="{{ route('posyandus.index') }}" method="GET" class="w-full">
                <div class="flex flex-wrap items-center gap-4 mb-4">
                    <div class="relative flex-1 w-full sm:w-auto min-w-0 sm:min-w-[300px]">
                        ...
                        <input type="text" name="search" id="search-input" value="{{ request('search') }}" placeholder="Cari posyandu berdasarkan nama atau alamat..." 
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50/30 transition duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-xs text-gray-500 truncate max-w-[200px]">{{ $item->alamat }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            @if(is_array($item->rw_diampu))
                                <div class="flex flex-wrap gap-1">
                                    @foreach($item->rw_diampu as $rw)
                                        <span class="px-2 py-0.5 bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold rounded-md">RW {{ $rw }}</span>
                                    @endforeach
                                </div>
                            @else
                                -
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            {{ $item->kaders_count }} Orang
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('kaders.index', ['posyandu_id' => $item->id]) }}" class="p-2 text-purple-600 hover:bg-purple-50 rounded-lg transition-colors" title="Kelola Kader">
                                    <i class="mdi mdi-account-tie text-xl"></i>
                                </a>
                                <a href="{{ route('posyandus.edit', $item) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                    <i class="mdi mdi-pencil text-xl"></i>
                                </a>
                                <form action="{{ route('posyandus.destroy', $item) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus posyandu ini beserta seluruh akun penggunanya?');">
                                    @csrf
                                    @method('DELETE')
