<x-layout title="Edit Jadwal Pelayanan">
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Edit Jadwal Pelayanan</h2>
        <p class="text-gray-500 text-sm mt-1">Lengkapi form berikut untuk mengubah jadwal pelayanan Posyandu.</p>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-200">
        <form action="{{ route('jadwals.update', $jadwal) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="hari_tanggal" value="{{ old('hari_tanggal', $jadwal->hari_tanggal) }}" required placeholder="Contoh: Senin, Minggu ke-1"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="kegiatan" value="{{ old('kegiatan', $jadwal->kegiatan) }}" required placeholder="Contoh: Bayi & Balita"
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all">
                    @error('kegiatan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="3" placeholder="Informasi tambahan atau yang perlu dibawa..."
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all">{{ old('keterangan', $jadwal->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
