...
<x-layout title="Data Ibu">
    <x-page-header 
        title="Data Ibu"
        subtitle="Daftar penduduk perempuan dengan status pernah atau sudah menikah"
    ...
    >
        <a id="export-excel" href="{{ route('ibus.export') }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-xs transition flex items-center gap-2 shadow-xs">
            ...
            <form id="filter-form" action="{{ route('ibus.index') }}" method="GET" class="space-y-4">
                ...
                <div class="flex flex-col md:flex-row justify-between gap-4 mb-4">
                    <div class="relative flex-1 min-w-[300px]">
                        ...
                        <input type="text" name="search" id="search-input" value="{{ request('search') }}" placeholder="Cari berdasarkan nama atau NIK..." 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($ibus as $item)
                    <tr class="hover:bg-blue-50/30 transition duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-semibold text-gray-900">{{ $item->nama }}</div>
                            <div class="text-xs text-gray-500">{{ $item->tempatlahir }}, {{ \Carbon\Carbon::parse($item->tanggallahir)->format('d/m/Y') }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-mono">{{ $item->nik }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-mono">{{ $item->no_kk }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ \Carbon\Carbon::parse($item->tanggallahir)->format('d/m/Y') }}</td>
    ...
    <script>
        ...
        document.getElementById('export-excel').addEventListener('click', function(e) {
            ...
            const exportUrl = "{{ route('ibus.export') }}?" + params.toString();
            
            // Start download
            window.location.href = exportUrl;
            
            // Reset button after a delay (approximation)
            setTimeout(() => {
