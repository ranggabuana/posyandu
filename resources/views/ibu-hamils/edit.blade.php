...
<x-layout title="Edit Ibu Hamil">
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight">Edit Data Ibu Hamil</h2>
            <p class="text-gray-500 mt-1">Perbarui informasi kehamilan di bawah ini.</p>
        </div>
    ...
    <form action="{{ route('ibu-hamils.update', $ibuHamil) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div>
                            ...
                            <select name="posyandu_id" required
                                ...
                                @foreach($posyandus as $p)
                                    <option value="{{ $p->id }}" {{ old('posyandu_id', $ibuHamil->posyandu_id) == $p->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="penduduk_id" id="select2-penduduk"
                                ...
                                @foreach($penduduks as $penduduk)
                                    <option value="{{ $penduduk->id }}" {{ old('penduduk_id', $ibuHamil->penduduk_id) == $penduduk->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="suami_id" id="select2-suami"
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all duration-200">
                                <option></option>
                                @if(old('suami_id', $ibuHamil->suami_id))
                                    @php
                                        $selectedSuamiId = old('suami_id', $ibuHamil->suami_id);
                                        $oldSuami = \App\Models\Penduduk::find($selectedSuamiId);
                                    @endphp
                                    @if($oldSuami)
                ...
                <script>
                    $(document).ready(function() {
                        ...
                        var initialSuamiId = "{{ old('suami_id', $ibuHamil->suami_id) }}";
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        <div>
                            ...
                            <input type="number" name="usia_kehamilan_minggu" value="{{ old('usia_kehamilan_minggu', $ibuHamil->usia_kehamilan_minggu) }}" placeholder="Contoh: 12" 
                        ...
                        <div>
                            ...
                            <input type="date" name="hpht" value="{{ old('hpht', $ibuHamil->hpht) }}" 
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all duration-200">
                        ...
                        <div>
                            ...
                            <input type="date" name="taksiran_persalinan" value="{{ old('taksiran_persalinan', $ibuHamil->taksiran_persalinan) }}" 
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all duration-200">
                        ...
                        <div>
                            ...
                            <input type="number" name="jumlah_kunjungan" value="{{ old('jumlah_kunjungan', $ibuHamil->jumlah_kunjungan) }}" 
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all duration-200">
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            <select name="status" 
                                ...
                                <option value="aktif" {{ old('status', $ibuHamil->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="melahirkan" {{ old('status', $ibuHamil->status) == 'melahirkan' ? 'selected' : '' }}>Melahirkan</option>
                                <option value="keguguran" {{ old('status', $ibuHamil->status) == 'keguguran' ? 'selected' : '' }}>Keguguran</option>
                                <option value="pindah" {{ old('status', $ibuHamil->status) == 'pindah' ? 'selected' : '' }}>Pindah</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="faktor_resiko" 
                                ...
                                <option value="umur < 20 th ATAU > 35 th" {{ old('faktor_resiko', $ibuHamil->faktor_resiko) == 'umur < 20 th ATAU > 35 th' ? 'selected' : '' }}>umur &lt; 20 th ATAU &gt; 35 th</option>
                                <option value="Jumlah anak > 4" {{ old('faktor_resiko', $ibuHamil->faktor_resiko) == 'Jumlah anak > 4' ? 'selected' : '' }}>Jumlah anak &gt; 4</option>
                                <option value="Jarak Hamil < 2 th" {{ old('faktor_resiko', $ibuHamil->faktor_resiko) == 'Jarak Hamil < 2 th' ? 'selected' : '' }}>Jarak Hamil &lt; 2 th</option>
                                <option value="lila kurang dr 23,5 cm" {{ old('faktor_resiko', $ibuHamil->faktor_resiko) == 'lila kurang dr 23,5 cm' ? 'selected' : '' }}>lila kurang dr 23,5 cm</option>
                                <option value="TB < 145 cm" {{ old('faktor_resiko', $ibuHamil->faktor_resiko) == 'TB < 145 cm' ? 'selected' : '' }}>TB &lt; 145 cm</option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea name="keterangan" rows="4" placeholder="Catatan tambahan..." 
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all duration-200">{{ old('keterangan', $ibuHamil->keterangan) }}</textarea>
