...
<x-layout title="Data Penduduk">
    <x-page-header 
        title="Data Penduduk"
        subtitle="Kelola data kependudukan Posyandu Melati Sehat Desa Banjar"
    ...
    >
        <a id="export-excel" href="{{ route('penduduks.export') }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-xs transition flex items-center gap-2 shadow-xs">
            <i class="mdi mdi-file-excel text-sm"></i> <span>Export Excel</span>
        </a>
        <a href="{{ route('penduduks.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs transition flex items-center gap-2 shadow-xs">
            <i class="mdi mdi-plus text-sm"></i> <span>Tambah Penduduk</span>
        </a>
    </x-page-header>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-4 border-b border-gray-200 bg-gray-50/50">
            <form id="filter-form" action="{{ route('penduduks.index') }}" method="GET" class="w-full">
                <div class="flex flex-wrap items-center gap-4 mb-4">
                    <div class="relative flex-1 w-full sm:w-auto min-w-0 sm:min-w-[300px]">
                        ...
                        <input type="text" name="search" id="search-input" value="{{ request('search') }}" placeholder="Cari berdasarkan nama, NIK, atau No KK..." 
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50/30 transition duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $item->no_kk }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2.5 py-1 text-xs font-medium rounded-full {{ $item->kelamin == 'laki-laki' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700' }}">
                                {{ ucfirst($item->kelamin) }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2.5 py-1 text-xs font-medium rounded-full {{ $item->status_kawin == 'kawin' ? 'bg-green-100 text-green-700' : ($item->status_kawin == 'belum kawin' ? 'bg-gray-100 text-gray-700' : 'bg-orange-100 text-orange-700') }}">
                                {{ ucfirst($item->status_kawin) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end gap-2">
                                ...
                                <form action="{{ route('penduduks.destroy', $item) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                    @csrf
                                    @method('DELETE')
