<x-layout title="Data Balita">
    <x-page-header 
        title="Data Balita (13-60 Bulan)"
        subtitle="Daftar pemantauan dan rekam medis balita usia 13 hingga 60 bulan"
        icon="mdi-baby"
        :breadcrumbs="[
            'Data Kesehatan' => null,
            'Data Balita' => null
    ...
    >
        <a id="export-excel" href="{{ route('balitas.export') }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-xs transition flex items-center gap-2 shadow-xs">
            <i class="mdi mdi-file-excel text-sm"></i> <span>Export Excel</span>
        </a>
        <a href="{{ route('balitas.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs transition flex items-center gap-2 shadow-xs">
            <i class="mdi mdi-plus text-sm"></i> <span>Tambah Balita</span>
        </a>
    </x-page-header>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-4 border-b border-gray-200 bg-gray-50/50">
            <form id="filter-form" action="{{ route('balitas.index') }}" method="GET" class="w-full">
                <div class="flex flex-wrap items-center gap-4 mb-4">
                    <div class="relative flex-1 w-full sm:w-auto min-w-0 sm:min-w-[300px]">
                        ...
                        <input type="text" name="search" id="search-input" value="{{ request('search') }}" placeholder="Cari berdasarkan nama balita atau nama ibu..." 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead>
                    <tr class="bg-gray-50">
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nama Balita</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Status Akta</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nama Ibu</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Posyandu</th>
                    ...
                    <tr class="hover:bg-blue-50/30 transition duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end gap-1.5">
                                <a href="{{ route('balitas.pemeriksaan', $item) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Penimbangan Bulanan">
                                    <i class="mdi mdi-scale-bathroom text-xl"></i>
                                </a>
                                <a href="{{ route('balitas.imunisasi', $item) }}" class="p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Catat Imunisasi">
                                    <i class="mdi mdi-needle text-xl"></i>
                                </a>
                                <a href="{{ route('balitas.edit', $item) }}" class="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit">
                                    <i class="mdi mdi-pencil text-xl"></i>
                                </a>
                                <form action="{{ route('balitas.destroy', $item) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus data ini?');">
                                    @csrf
                                    @method('DELETE')
