...
class RemajaExport implements FromQuery, WithHeadings, WithEvents, WithCustomStartCell, ShouldAutoSize, WithMapping, WithColumnFormatting
{
    protected $filters;
    ...
    public function query()
    {
        ...
        $query = Penduduk::query()
            ->select('nama', 'nik', 'kelamin', 'tanggallahir', 'dusun', 'rw', 'rt', 'telepon')
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function(BeforeSheet $event) {
                ...
                $event->sheet->setCellValue('A1', 'LAPORAN DATA REMAJA (USIA ' . $minAge . ' - ' . $maxAge . ' TAHUN)');
            ...
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $dataRange = 'A5:I' . $lastRow;
                $event->sheet->getStyle($dataRange)->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);
                
                // Align content
                $event->sheet->getStyle('C6:G' . $lastRow)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            },
        ];
    }
}
