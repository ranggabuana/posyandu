...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $totalBalita = BayiBalita::whereRaw("TIMESTAMPDIFF(MONTH, tanggal_lahir, CURDATE()) > 12 AND TIMESTAMPDIFF(MONTH, tanggal_lahir, CURDATE()) <= 60")->count();
        $totalRemaja = Penduduk::whereRaw("TIMESTAMPDIFF(YEAR, tanggallahir, CURDATE()) BETWEEN ? AND ?", [$remajaMin, $remajaMax])->count();
        $totalLansia = Penduduk::whereRaw("TIMESTAMPDIFF(YEAR, tanggallahir, CURDATE()) >= ?", [$lansiaMin])->count();
        $totalWus = Penduduk::where('kelamin', 'perempuan')->whereRaw("TIMESTAMPDIFF(YEAR, tanggallahir, CURDATE()) BETWEEN ? AND ?", [$wusMin, $wusMax])->count();
        $totalPus = Pus::count();
        ...
        $remajaExamCount = PemeriksaanRemaja::count();
        $lansiaExamCount = PemeriksaanLansia::count();

        // 5. Stunting & Nutrition Summary (Balita) with Detailed Person Lists
        $latestBalitaExams = PemeriksaanBalita::with('bayiBalita.penduduk')
        ...
        $latestRemajaExams = PemeriksaanRemaja::with('remaja.penduduk')
        ...
        $latestLansiaExams = PemeriksaanLansia::with('lansia.penduduk')
        ...
        $recentBalitaExams = PemeriksaanBalita::with('bayiBalita.penduduk')->orderBy('tanggal_pemeriksaan', 'desc')->take(5)->get();
        $recentLansiaExams = PemeriksaanLansia::with('lansia.penduduk')->orderBy('tanggal_pemeriksaan', 'desc')->take(5)->get();
        $recentRemajaExams = PemeriksaanRemaja::with('remaja.penduduk')->orderBy('tanggal_pemeriksaan', 'desc')->take(5)->get();
